...
export const getQueuePosition = query({
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    //   get total number of peoples ahead in line
    const peoplesAhead = await ctx.db
      .query("waitingList")
...
// Mutation to process waiting list queue and offer ticket to next person and check current availability considering purchase tickets and active offer
export const processQueue = mutation({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    // Create time-limited offer for selected user
    const now = Date.now();

    for (const user of waitingUsers) {
      // update the waiting list entry to offer
      await ctx.db.patch(user._id, {
        status: WAITING_LIST_STATUS.OFFERED,
        offerExpiresAt: now + DURATIONS.TICKET_OFFER,
      });

      // schedule the expiration job for the offer
